...
head('Данни, които чакат клиента');
{
  ...
  // От 16.09.2026 празното поле НЕ се рисува — рамките „чака се“ са махнати по
  // искане на Добо. Тоест на екрана няма нищо, което да напомня; този ред е
  // единственото напомняне и затова е по-строг, отколкото изглежда.
  if (empty.length) warn(`празни полета: ${empty.join(', ')}`, 'редът просто не се показва — НЯМА кой да напомни от сайта');
